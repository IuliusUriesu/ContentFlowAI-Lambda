...
import { EditGeneratedContentPieceBodySchema } from "../../models/api/EditGeneratedContentPieceBody";

const editGeneratedContentPiece = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    ...
    const parsedBody = EditGeneratedContentPieceBodySchema.safeParse(body);
    ...
    try {
        ...
        const updatedGeneratedContentPiece = await dynamoDbService.updateGeneratedContentPieceContent({
            userId: sub,
...
export default editGeneratedContentPiece;
